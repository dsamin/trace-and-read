...
> **Build order:** App 3 of 5 in a shared-chassis learning slate, and the one that takes on writing — where handwriting and reading first come together.

## What it is

Trace & Read is an iPad app for early learners (preschool / pre-K, roughly ages 3–5) that teaches children how to **form letters by hand** and then **bridges that act of writing directly into reading the word they just wrote**. It is the third of five small learning apps built on a shared chassis. Handwriting and reading develop together at this age, so this app takes the act of writing and wires it straight to the reading goal: the moment a child finishes tracing `s-u-n`, the app blends the sounds aloud with them — "s-u-n… *sun!*" — and shows a picture of the sun. **Writing the word is the unlock for reading it.**

It is built on three rules the whole app slate shares. It is **audio-first** (an early learner can't read a menu, so every instruction is spoken). It is **errorless** (a wrong stroke is never a buzzer or a fail screen — it gently re-models the correct stroke and lets the child try again). And it is **calm** (muted palette, soft transitions, one warm human voice, no confetti, no streaks, no timers, no scores). There are no ads, no in-app purchases, no accounts, and no network — everything runs on-device.

## Who it's for

**Early learners (preschool / pre-K, roughly ages 3–5).** Bright and curious, at the stage where handwriting, letter formation, and word recognition are all developing together. A child this age may not yet read, so nothing in the app may require reading to operate it. Attention spans are short: the app must reward fast, keep the tone gentle, and always offer a low-pressure mode for when energy is low.

The design optimizes for the developmental realities of this age group — starting with one of the most motivating things a young child can write: **the child's own name (entered by a parent).**
...
2. **Reading via the write-then-read bridge** — every word a child writes is immediately blended aloud and shown as a picture, so forming the letters becomes the path to reading the word. This is the core pedagogical bet of the app.
...
1. **Day one opens on the child's own name.** The most motivating possible first word. After meeting their name, the child picks a CVC word (cat, dog, sun…) from a spoken, picture-based chooser.
2. **Per letter, a 3-step fade (LetterSchool-style):**
 ...
 - **Guided trace** — the child traces over the on-screen guide. Stroke tolerance is **very generous early** and tightens only as mastery rises.
 - **Free-write from memory** — the guide disappears and the child writes the letter **once, unscaffolded**. This is the step most tracing apps skip, and it's the one that builds durable formation instead of guide-dependence. Hints fade back in only if the child stalls.
3. **Each completed letter voices its sound** ("sss"), reinforcing letter→sound mapping as the child goes.
4. **On finishing all three letters, the bridge fires:** the app blends the sounds aloud *with the child* — "s-u-n… *sun!*" — and reveals the picture. The word they just wrote is now a word they can read.
5. **A wrong action never fails the child.** If a stroke goes wrong, the voice stays encouraging ("let's try that line again") and the correct stroke is re-modeled. There is no buzzer, no red X, no fail screen.
6. **"Just trace, no judging" free mode is one tap away** — for low-energy days, the child can trace with zero validation and zero pressure.

## Screens

- **Name screen (day one / home anchor)** — the child's name laid out as oversized guided letters; the entry point and the emotional hook.
...
- **Parent area (behind a parent gate)** — settings, word/letter selection, left-handed mirrored-demo toggle, and a **replay of how each letter was formed**. Reached via press-and-hold then drag, so a young child can't wander in.

## Key design decisions

These are the make-or-break calls specific to this app:

- **Robust real-time stroke validation is the hardest problem — this is the #1 risk.** Detecting stroke *order* and *direction* (not just "did the child color inside the lines") via PencilKit / Core Graphics path-matching must be **strict enough to teach correct formation yet forgiving enough not to frustrate a young child.** The chosen answer: start tolerance very generous, tighten with mastery, and **test with real children early and often.** Get this wrong in either direction and the app fails — too strict and the child quits, too loose and it doesn't teach. Build the validator behind a clean interface with tunable tolerance constants so the strictness can be dialed without touching the UI.
...
- **Name-first personalization.** One of the strongest motivational levers at this age is the child's own name; it leads everything.
- **Big targets, finger-first.** Oversized guide dots and thick strokes; stylus is welcome but never required. Left-handed mirrored demo is a planned toggle so the demo hand doesn't occlude the child's writing hand.
...
1. **Name-first screen** — the child's name as guided letters; the day-one entry point.
...
- [ ] Day one opens on **the child's own name** rendered as oversized guided letters.
- [ ] After the name, the child can pick a CVC word from a **spoken, picture-based chooser** that requires no reading.
...
- [ ] A wrong stroke **never** produces a buzzer / red X / fail screen — it re-models the stroke and the voice line stays **encouraging**, then lets the child retry.
